...
app.use(async (req, res, next) => {
  if (req.cookies.token) {
    ...
    if (token.user_id) {
      let [user] = await global.db.query("SELECT * FROM users WHERE id = ?", [
        token.user_id,
      ]);
      req.user = user[0];
...
const modulesFolders = (await readdir(modulesPath, { withFileTypes: true }))
...
app.listen(global.config.port, function () {
  console.log(`Server started on port ${global.config.port}`);
});
